// CircularGallery — vanilla JS adaptation of the React Bits component
// Uses ogl for WebGL; loaded as an ES module via import map
...
// ── Title (text label below each image) ──
...
class GalleryApp {
  constructor(container, options = {}) {
    const {
      ...
      bend = 3,
      textColor = '#1a1a1a',
      font = 'bold 30px Figtree, system-ui, sans-serif',
      scrollSpeed = 2,
      scrollEase = 0.05
    } = options;
  ...
  createMedias(items, bend, textColor, font) {
    const defaultItems = [
    ...
    this.medias = this.mediasImages.map((data, index) => new Media({
      ...
      viewport: this.viewport, bend, textColor, font
    }));
  }
...
// ── Public API ──
// Creates a CircularGallery instance inside the given container element.
// Returns the GalleryApp instance so it can be destroyed later.
// Usage: createCircularGallery(document.getElementById('my-gallery'), { items: [...] })
