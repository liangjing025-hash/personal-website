...
(function () {
 ...
 var APP_DATA = deepMerge(DEFAULT_DATA, _predefinedData || {});
  ...
  function renderExperience() {
    ...
    // ═══ 项目经验 — 四图横排 hover 展开 ═══
    h += renderProjectCards(projItems);
  ...
  function renderProjectCards(items) {
    ...
    h += '  <h2 class="proj-cards-title">' + esc('项目经验') + '</h2>';
    ...
    items.forEach(function(item) {
      ...
      h += '          <div class="proj-card-name">' + esc(item.name || '') + '</div>';
      h += '          <div class="proj-card-role">' + esc(item.role || '') + '</div>';
      if (item.details && Array.isArray(item.details)) {
        h += '          <ul class="proj-card-list">';
        item.details.forEach(function(d) { h += '            <li>' + esc(d) + '</li>'; });
        h += '          </ul>';
      }
      h += '        </div>';
      h += '      </div>';
